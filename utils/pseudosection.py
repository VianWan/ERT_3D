        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if not log_switch:
                    ...
                    fc_bar.set_label(" Apparent Resistivity ($\log_{10}^{ρ} \Omega\cdotp$m)", fontdict={
                        'weight': 'normal', 'size': 20}, labelpad=10)
...
# althought we only use A and M electrode loaction, but for sepcification, still remian B and N position parmameter.
...
# ----- #
    electrode_x = loc_a.add(loc_m.add(loc_n) / 2) / 2
    serinedez = np.r_[np.diff(np.r_[-1, np.argwhere(np.diff(loc_a) > 0).ravel(), loc_a.size - 1])]
...
def dipole_dipole(
                  loc_a:'electrodeA_loction' = None,
                  loc_b:'electrodeB_location' = None,
                  loc_m:'electrodeM_loction' = None,
                  loc_n:'N_loction' = None,
                  c_values:'apperent reistivity'=None,
                  interp: bool = False,
                  log_switch: bool = False,
) -> Tuple[plt.Axes,plt.Axes,plt.Axes]:

    electrode_x = (loc_m.add(loc_n)/2+loc_a.add(loc_b)/2)/2
    serinedez = np.r_[np.diff(np.r_[-1, np.argwhere(np.diff(loc_a) > 0).ravel(), loc_a.size - 1])]
    electrode_z = np.array([j for i in serinedez for j in range(0, i)])

    
    if interp:
        [electrode_x,electrode_z,c_values] = interp_electrode(electrode_x,electrode_z,c_values,step=0.3 * (loc_a[0] + loc_a[1])) # 0.5 is default u can change free
    if log_switch:
        c_values=np.log10(c_values)
...
-------------------------------------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------------------------------------
    ...
    assert survey_type in fun_type,'survey_type is illegal parameters'
...
if __name__ == '__main__':
    ...
    plot_pseudosection(loc_a,loc_b, loc_m, loc_n, obs_voltage, interp=True, log_switch=True,survey_type='pole-pole')
    plt.show()
